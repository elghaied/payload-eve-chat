...
- **Hardware:** NVIDIA GPU available; Docker services configured for GPU.
...
- **Topology:** Two specialized Docker services (Approach A) — `speaches` for STT, `Kokoro-FastAPI` for TTS — each independently swappable.
...
Two new GPU services on the default compose network, with ports published to host so a host-run `pnpm devsafe` reaches them at `localhost` (same pattern as Ollama at `localhost:11434`):
...
- These services have **no `depends_on`** relationship with `payload` — voice is optional and they start independently.
...
Extend `EveConfig` and `getEveConfig()` with an audio block, following the existing base-URL + model + optional-key shape. New env keys (added to `.env.example`):
...
Added `EveConfig` fields (all resolved with defaults so they are always present):
...
Resolution rules:
- `voiceEnabled` = `env.VOICE_ENABLED === 'true'` (default `false` when unset, so production must opt in).
- Base URLs / model / voice / format fall back to the defaults above.
- No throw when keys are absent (local servers need none); cloud usage sets the keys and base URLs.
...
2. If `!eveConfig.voiceEnabled` → 503 `{ error: 'Voice disabled' }`.
...
Behavior:
- `start()`: `getUserMedia` (mic) + init `@ricky0123/vad-web` (`MicVAD`). On permission denial set `error` and stay `idle`.
...
- **Assistant streaming** (driven by `assistantText` + `status`): feed a `createSentenceStreamer()`; for each emitted sentence `POST /api/eve/speak` and enqueue audio in a **sequential playback queue** (one `Audio`/`AudioContext` source at a time, FIFO). State `speaking` while the queue is non-empty. On `status` leaving `streaming`, call `streamer.flush()` and enqueue the remainder.
...
- Add a call-style toggle button (mic / hang-up) to the existing `PromptInputFooter` (next to submit). Visible only when `voiceEnabled` (passed from the server view as a prop) and hidden/disabled otherwise.
- Wire `useVoice`:
...
- Show `error` inline (small notice) and a compact state indicator (listening / thinking / speaking).
- `voiceEnabled` is read server-side (e.g. in `EveView.tsx`, which already runs on the server) from `getEveConfig()` and passed down — the client never receives base URLs/keys.
...
- **Mic permission denied:** `useVoice` sets `error`, stays idle; text chat unaffected.
- **`VOICE_ENABLED=false`:** voice button hidden; routes return 503; nothing else changes.
- **STT/TTS service down or non-2xx:** route returns 503 with a readable message; `useVoice` surfaces it inline and stays in text mode (no crash, no broken queue).
...
- **config:** `getEveConfig` audio fields — defaults, `VOICE_ENABLED` parsing, env overrides, key pass-through.
...
- `docker-compose.yml` — add `stt`, `tts` services + `hf-cache` volume. *(modify)*
- `.env.example` — add the voice env block. *(modify)*
...
- `src/components/eve/EveView.tsx` — pass `voiceEnabled` prop from `getEveConfig`. *(modify)*
